...
class CronRecord():
    def __init__(self, jobID, time, frequency):
        self.jobID = jobID
        self.time = time
...
class HiveDataStore():
    ...
    def writeCronToCsv(self, filename, records):
        with open(filename+'.csv', 'wb') as csvfile:
            writer = csv.writer(csvfile, delimiter=',',
                            quotechar='|', quoting=csv.QUOTE_MINIMAL)
            writer.writerow(['JobID', 'Time', 'Frequency'])
            for record in records:
                writer.writerow([record.jobID, record.time, record.frequency])